...
// This is not the official NEMO Version, please for official version follow the link ABOVE
...
#define FRONT_BUTTON_PIN 5  // Assuming the front button is the COPY button
#define SIDE_BUTTON_PIN 6   // Assuming the side button is the SEND button
#define BACK_BUTTON_PIN 7

IRrecv irReceiver(IR_RECEIVER_PIN);
decode_results copiedIRCode;
...
void setup() {
  ...
  // Initialize other setup routines as needed...
}

void loop() {
  // Existing Nemo features...
  // ...

  // New features for copying and sending IR signals
  if (isButtonPressed(FRONT_BUTTON_PIN)) {
    // IR Copy functionality
    Serial.println("Press a button on the remote to copy an IR signal...");
    if (irReceiver.decode(&copiedIRCode)) {
      Serial.println("IR Signal Copied!");
      irReceiver.resume();
    }
  }

  if (isButtonPressed(SIDE_BUTTON_PIN)) {
    // IR Send functionality
    if (copiedIRCode.decode_type != UNKNOWN) {
      Serial.println("Sending Copied IR Signal...");
      irsend.sendRaw(copiedIRCode.rawbuf, copiedIRCode.rawlen, copiedIRCode.decode_type);
    } else {
      Serial.println("No IR Signal Copied Yet.");
    }
    waitForButtonRelease(SIDE_BUTTON_PIN);
  }

  // Continue with existing Nemo features...
  // ...
}
...
bool isButtonLongPressed(int buttonPin) {
...
String days[7]={"Segunda-feira","Terca-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sabado","Domingo"};
#endif
...
void new_clock_loop() {
 ...
 DISP.setFreeFont(&DSEG7_Classic_Bold_30);
 ...
 if(mi.length()<2)
 mi="0"+mi;
 DISP.drawString(ho+":"+mi,2,46);
 H=int(RTC_TimeStruct.Hours);
 M=int(RTC_TimeStruct.Minutes);
 }
  
  //PRINT Week Day
  DISP.drawString(days[RTC_DateStruct.WeekDay-1]+"    ",4,0,2);
  DISP.setTextColor(grey,BGCOLOR);
  //PRINT DATE
  DISP.drawString(String(RTC_DateStruct.Date)+"/"+String(RTC_DateStruct.Month)+"/"+String(RTC_DateStruct.Year),4,20,2);
  
  //PRINT seconds
  se=String(RTC_TimeStruct.Seconds);
    if(se.length()<2)
  se="0"+se;
  DISP.drawString(se,112,57,4);

  //Displays Battery Voltage - 4.15 > Charging < Not
  //Displays Battery Level
  float batVoltage = M5.Axp.GetBatVoltage();
  float batPercentage = ( batVoltage < 3.2 ) ? 0 : ( batVoltage - 3.2 ) * 100;
  DISP.drawString(String(batPercentage, 0)+"%",116,17,2);
  DISP.setTextColor(FGCOLOR,BGCOLOR);
  //DISP.drawString(String(batVoltage)+"V",114,17,2);

  if ( batVoltage > 4.15 ) {
  DISP.drawString("charging",106,0,2);
  } else {
    DISP.drawString("unpluged",106,0,2);
  }
}
